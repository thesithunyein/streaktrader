      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-5 sm:px-8 pt-6 pb-12">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Your complete trading performance</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          {[
            ...
            { icon: BarChart3, label: "Total Trades", value: totalTrades.toString(), color: "text-slate-900" },
            ...
            <Animate key={stat.label} delay={i * 50}>
              <div className="glass rounded-[16px] p-4 sm:p-5 text-center">
                ...
                <div className="text-[10px] sm:text-xs text-slate-400 uppercase tracking-wider">{stat.label}</div>
              </div>
            </Animate>
          ))}
        </div>
        ...
        <div className="glass rounded-[20px] p-4 sm:p-6 mb-6">
          <h2 className="text-base sm:text-lg font-bold text-slate-900 mb-4">Streak Progress</h2>
          {trades.length > 0 ? (
        ...
        <div className="glass rounded-[20px] p-4 sm:p-6">
          <h2 className="text-base sm:text-lg font-bold text-slate-900 mb-4">Recent Trades</h2>
          {trades.length > 0 ? (
            <div className="space-y-2">
              {trades.slice(0, 20).map((trade, i) => (
                <div
                  key={trade.id}
                  className="opacity-0 animate-fade-left flex items-center gap-3 sm:gap-4 p-3 rounded-[12px] bg-slate-50 border border-slate-100 hover:bg-slate-100/50 transition-colors"
                  style={{ animationDelay: `${i * 30}ms` }}
                >
                  <div className={`w-8 h-8 rounded-[8px] flex items-center justify-center ${
                    trade.result === "WIN" ? "bg-[#16a34a]/10" : trade.result === "LOSE" ? "bg-[#dc2626]/10" : "bg-slate-100"
                  }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[10px] text-slate-400 mt-0.5">
                      ...
                      {trade.shieldUsed && <span className="ml-2 text-[#2563eb]">· Shield</span>}
                    </div>
                  </div>
                  ...
                  <div className={`text-sm font-mono font-bold shrink-0 ${
                    trade.result === "WIN" ? "text-[#16a34a]" : trade.result === "LOSE" ? "text-[#dc2626]" : "text-slate-400"
                  }`}>
            ...
            <div className="text-center py-12 text-slate-400">
              ...
              <p className="text-xs text-slate-300 mt-1">Your trade history will appear here</p>
