...
import {
  ...
  RefreshCw, ExternalLink, Users, TrendingUp,
} from "lucide-react";
...
function ScoreBar({ score }: { score: number }) {
  const color =
    score >= 80 ? "bg-[#2563eb]" : score >= 60 ? "bg-[#16a34a]" : score >= 40 ? "bg-[#f59e0b]" : "bg-white/20";
  return (
...
function RankBadge({ rank }: { rank: number }) {
  ...
  if (rank === 2) return <Medal className="w-5 h-5 text-white/40" />;
  if (rank === 3) return <Award className="w-5 h-5 text-[#f59e0b]/60" />;
  return <span className="text-sm font-bold text-white/30 w-5 text-center">{rank}</span>;
}
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-5 sm:px-8 pt-6 pb-12">
        ...
        {loading && entries.length === 0 ? (
          ...
          <div className="glass rounded-[20px] p-12 text-center">
            ...
            <h2 className="text-lg font-bold text-white mb-2">No traders yet</h2>
          ...
          <>
                  ...
                  return (
                    <Animate key={entry.address} delay={rank * 100} direction="scale">
                      <div className={`glass rounded-[20px] p-4 sm:p-5 text-center ${isFirst ? "ring-2 ring-[#f59e0b]/30 -mt-4" : ""}`}>
                        <div className={`w-12 h-12 rounded-full mx-auto mb-3 flex items-center justify-center ${isFirst ? "bg-[#f59e0b]/10" : "bg-white/5"}`}>
                          <RankBadge rank={rank} />
                        </div>
                        <div className="text-sm font-bold text-white mb-0.5 font-mono">{truncateAddress(entry.address)}</div>
                        <div className="text-2xl font-black font-mono text-gradient mb-1">{entry.score}</div>
                        <div className="text-[10px] text-white/30 uppercase tracking-wider mb-3">Score</div>
            ...
            <div className="glass rounded-[20px] overflow-hidden">
              <div className="px-5 py-3 border-b border-white/[0.06]">
                <div className="grid grid-cols-12 gap-2 text-[10px] font-semibold text-white/30 uppercase tracking-wider">
              ...
              {entries.map((entry, i) => (
                <div key={entry.address} className="px-5 py-3 border-b border-white/[0.03] last:border-0 hover:bg-white/[0.02] transition-colors">
                  <div className="grid grid-cols-12 gap-2 items-center">
                    <div className="col-span-1 flex justify-center"><RankBadge rank={i + 1} /></div>
                    <div className="col-span-4">
                      <div className="text-sm font-bold text-white font-mono">{truncateAddress(entry.address)}</div>
                      <div className="text-[10px] text-white/30">{entry.totalTrades} trades</div>
                    </div>
                    <div className="col-span-2 text-center">
                      <div className="flex items-center justify-center gap-1">
                        <Flame className="w-3 h-3 text-[#f59e0b]" />
                        <span className="text-sm font-bold font-mono text-white">{entry.bestStreak}x</span>
                      </div>
                    </div>
                    <div className="col-span-2 text-center">
                      <span className={`text-sm font-bold font-mono ${entry.winRate >= 70 ? "text-[#16a34a]" : entry.winRate >= 50 ? "text-white" : "text-[#dc2626]"}`}>
                        {entry.winRate}%
                      </span>
                    </div>
        ...
        <div className="mt-6 text-center">
          <p className="text-xs text-white/30">Scores computed by <span className="font-semibold text-accent">ScoreOracle</span> contract</p>
          {lastUpdated && <p className="text-[10px] text-white/20 mt-1">Last updated: {lastUpdated}</p>}
        </div>
      </main>
      <Footer />
    </div>
