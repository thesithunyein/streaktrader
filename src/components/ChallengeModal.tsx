  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        ...
        className="relative z-10 bg-white rounded-3xl p-6 max-w-md w-full mx-4 border border-slate-200 shadow-2xl">
        ...
        {!challengeCreated ? (
          <>
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-900">Create Challenge</h3>
                <p className="text-sm text-slate-500">Challenge a friend to predict the same market</p>
              </div>
            </div>
            ...
            <div className="bg-slate-50 rounded-xl p-4 mb-5 border border-slate-200">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-sm font-semibold text-slate-900">{market.underlying}</span>
              ...
              <div className="flex gap-2">
                {presets.map((p) => (
                  <motion.button key={p} whileTap={{ scale: 0.95 }} onClick={() => setStake(p)}
                    className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-colors ${stake === p ? "bg-accent/10 text-accent border border-accent/20" : "bg-slate-50 text-slate-500 border border-slate-200 hover:border-accent/20"}`}>
                    {p}
                  </motion.button>
                ))}
              </div>
          ...
          <>
            ...
            <div className="text-center mb-6">
              ...
              <h3 className="text-lg font-bold text-slate-900 mb-1">Challenge Created!</h3>
              <p className="text-sm text-slate-500">Share this link with your opponent</p>
            </div>

            {/* Challenge link */}
            <div className="bg-slate-50 rounded-xl p-4 mb-5 border border-slate-200">
              <div className="text-xs text-slate-500 mb-2">Challenge Link</div>
              <div className="flex items-center gap-2">
                <div className="flex-1 text-sm font-mono text-slate-900 truncate bg-white rounded-lg px-3 py-2 border border-slate-200">
                  {getChallengeLink()}
                </div>
                <motion.button whileTap={{ scale: 0.95 }} onClick={handleCopyLink}
                  className={`p-2 rounded-lg transition-colors ${copied ? "bg-up/10 text-up" : "bg-slate-100 text-slate-500 hover:bg-slate-200"}`}>
            ...
            <div className="bg-accent/5 rounded-xl p-4 mb-5 border border-accent/15">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-slate-500">Your pick</span>
                <span className={`text-sm font-bold ${side === "UP" ? "text-up" : "text-down"}`}>{side}</span>
              </div>
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-slate-500">Opponent picks</span>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-bold text-slate-900">{stake} tUSDC each</span>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                ...
                className="flex-1 py-3 rounded-xl bg-slate-100 text-slate-900 font-bold text-sm flex items-center justify-center gap-2">
            ...
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
              onClick={onClose}
              className="w-full mt-3 py-3 rounded-xl text-sm font-bold text-slate-500 hover:text-slate-900 transition-colors">
              Done
            </motion.button>
          </>
        )}
